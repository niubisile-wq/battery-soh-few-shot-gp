...
OUT=ROOT/'外部泛化方案二审计_20260913'; ZIP=ROOT/'数据集/Stanford_new_untouched/HNEI.zip'; RELEASE=ROOT/'模块研发/results/m4_random_function_candidate_v1/folds'
...
def load_cell(o,n):
 ...
 q=np.asarray(caps,float); return Cell(n,'HNEI',n,np.asarray(xs,np.float32),q/q[0],np.asarray(nums,np.float32),float(q[0]),nom,n)
def main():
  ...
  for ix,n in enumerate(names,1):
   ...
   for fp in FOLDS:
    ...
    for model,p in [('B',pB),('B1234',pFull)]:
     m=metrics(y,p); rows.append(dict(dataset='HNEI',cell_id=n,fold=fp.parts[-3],model=model,query_count=len(y),**m))
   print(ix,len(names),n,'eligible',len(c.y),'elapsed',round(time.time()-t0,1),flush=True)
 with (OUT/'HNEI冻结模型外部测试_逐轨迹.csv').open('w',newline='') as f:
  w=csv.DictWriter(f,fieldnames=rows[0]);w.writeheader();w.writerows(rows)
 agg=[]
 for model in ['B','B1234']:
  rr=[r for r in rows if r['model']==model]
  agg.append(dict(dataset='HNEI',model=model,trajectories=len(rr),cells=len(set(r['cell_id'] for r in rr)),query_points=sum(r['n'] for r in rr),mae=float(np.mean([r['mae'] for r in rr])),rmse=float(np.mean([r['rmse'] for r in rr])),p95=float(np.mean([r['p95'] for r in rr])),bias=float(np.mean([r['bias'] for r in rr]))))
 (OUT/'HNEI冻结模型外部测试_汇总.json').write_text(json.dumps({'status':'COMPLETE_HISTORICALLY_EXPOSED_STRESS_TEST','dataset':'HNEI','cells':cells,'eligible_query_points':eligible,'folds':[str(x.relative_to(ROOT)) for x in FOLDS],'aggregation':'equal mean over cell-fold trajectories; six source-fold models, no new ensemble','results':agg,'independence':'FAIL_SNL_HISTORICALLY_SCORED_IN_MULTIDATASET_HI_MATRIX','zip_sha256':hashlib.sha256(ZIP.read_bytes()).hexdigest()},indent=2),encoding='utf8')
