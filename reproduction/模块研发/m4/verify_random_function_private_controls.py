"""Independent target arithmetic, all40 SOH groups and paired intervals."""
import csv
import itertools
import json
...
def mapped(g, f):
    return g + ('_' + f if '4' in g and f != 'function_private' else '')
...
def main():
    ...
    assert request['code_sha256'] == sa.digest(HERE / 'score_random_function_private_controls.py')
    ...
    assert len(rows) == len(lookup) == 14600
            ...
            for c in test:
                ...
                for f, model in models.items():
                    ...
                    if f.startswith('function_'):
                        ...
                        q = independent_predict(model, visible, 'private' if f == 'slope_private' else 'shared', point_mean=True)
                ...
                with np.load(root / tail) as z, np.load(old / tail) as control:
                    ...
                    for g in parents:
                        errors['control'] = max(errors['control'], float(np.max(abs(z[mapped(g + '4', 'function_private')] - (.5 * control[g] + .5 * control[g + '4_function_private'])))))
    ...
    assert len(table) == len(summary['table']) == 120
    ...
    sa.write_json(root / 'verification.json', dict(status='PASS', rows=14600, edges=128, errors=errors, paired_comparisons=pairs,
        ...
        helper_hashes={n:sa.digest(HERE / n) for n in ('audit_random_function.py', 'audit_conditional_mixed.py', 'freeze_random_function_private_controls.py')},
        limits='Independent target arithmetic and equal-domain metrics, not new source training. Paired intervals uncorrected for repeated development selection. All four controls have exact private0.5 kernel/gain. Private vs shared within source fit isolates target private component; private cross-source comparison changes both source and target covariance. No independent generalization claim.'))
