"""Frozen four-family smooth-function screen, original parents untouched."""
import json
...
FAMILIES = ('function_private', 'function_shared', 'slope_private', 'slope_shared')


def group_name(g, family):
    return g + ('_' + family if '4' in g and family != 'function_private' else '')


def branch_prediction(model, family, cell):
    adapter = ConditionalFunction if family.startswith('function_') else ConditionalMixed
    if family.endswith('_private'): return adapter(model, True).predict(cell)
    if family.endswith('_shared'): return adapter(model, False).predict(cell)
    return .5 * (adapter(model, True).predict(cell) + adapter(model, False).predict(cell))


def main():
    results = HERE.parent / 'results'; root = results / 'm4_random_function_private_controls_v1'
    ...
    sa.write_json(root / 'score_request.json', dict(frozen_sha256=sa.digest(root / 'frozen_selections.json'),
        ...
        helper_hashes={n:sa.digest(HERE / n) for n in ('random_function_branch.py', 'random_function_gp.py', 'conditional_mixed.py', 'freeze_random_function_private_controls.py')}))
    parent = results / 'm3_waveform_candidate_v1'; old = results / 'm4_random_function_screen_v1'
    ...
    with threadpool_limits(limits=1):
        for fold in frozen['selections']:
            ...
            for c in test:
                ...
                with np.load(old / tail) as z:
                    ...
                    for g in PARENTS:
                        ...
                        control_error = max(control_error, float(np.max(abs(pp[group_name(g + '4', 'function_private')] - (.5 * z[g] + .5 * z[g + '4_function_private'])))))
            ...
            print(fold['fold'], 'four-family random-function screen', flush=True)
    assert len(rows) == 14600 and max(boundary, control_error) < 1e-8
    ...
    sa.write_json(root / 'summary.json', dict(status='SCREEN_COMPLETE_NOT_ADOPTED', primary='function_private', rows=len(rows), table=table,
        failures=failures, gates=gates, boundary=boundary, control_replay_error=control_error, parent_metric_error=parent_error,
        limits='Four exact private0.5 matched covariance/mode controls, no new selection. No GP refits, original parents unchanged. Independent target replay and uncertainty pending. Repeated development cohorts, not independent confirmation.'))
