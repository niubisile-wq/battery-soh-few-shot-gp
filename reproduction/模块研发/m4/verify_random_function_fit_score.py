...
def mapped(g, f):
    return g + ('_' + f if '4' in g and f != 'init_1.0' else '')
...
def main():
    ...
    assert request['code_sha256'] == sa.digest(HERE / 'score_random_function_fit.py')
        ...
        for fold in frozen['selections']:
            ...
            for c in test:
                ...
                for f, model in models.items():
                    ...
                    q = independent_function_predict(model, visible, True)
                ...
                with np.load(root / tail) as z, np.load(old / tail) as control:
                    ...
                    for g in parents:
                        errors['control'] = max(errors['control'], float(np.max(abs(z[mapped(g + '4', 'init_1.0')] - control[g + '4']))))
    ...
    sa.write_json(root / 'verification.json', dict(status='PASS', rows=17520, edges=160, errors=errors, paired_comparisons=pairs,
        ...
        helper_hashes={n:sa.digest(HERE / n) for n in ('audit_random_function.py', 'audit_conditional_mixed.py', 'freeze_random_function_fit.py')},
        limits='Independent target arithmetic and equal-domain metrics, not new source training. Paired intervals uncorrected for repeated development selection. All five source-initialization fits retain private0.5 kernel/gain and target mode. Source optimization sensitivity, not whole-pipeline retraining; no best-run selection. No independent generalization claim.'))
