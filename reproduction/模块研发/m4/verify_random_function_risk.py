...
def main():
    results=HERE.parent/'results';root=results/'m4_random_function_risk_v1'
    source=results/'m4_random_function_screen_v1';risk=results/'m3_waveform_stability_v2'
    ...
    assert sa.digest(HERE/'random_function_risk.py')==protocol['code_sha256']
    assert protocol['source_audit_sha256']==sa.digest(source/'verification.json')
    assert protocol['sensitivity_audit_sha256']==sa.digest(results/'m4_random_function_sensitivity_v1/verification.json')
    ...
    gains={s['fold']:s['choices']['function_private']['selected']['gain'] for s in choices}
    ...
    for ds in ('XJTU','MATR','Tongji'):
        for c in sa.load_cells(ds):
            ...
            with np.load(source/'folds'/folder/'predictions'/(c.id+'.npz')) as z:
                y=z['y'].copy();q=z['branch_function_private'].copy()
    ...
    for step in protocol['steps']:
        counts[str(step)]={k:0 for k in ('independent','full','all_m4_additions','all32')};all_pass[str(step)]=0
        ...
        for seed in protocol['seeds']:
            ...
            counts[str(step)]['independent']+=gates['B4<B']
            counts[str(step)]['full']+=gates['B1234<B123']
            counts[str(step)]['all_m4_additions']+=all(gates[g+'4<'+g] for g in groups)
            counts[str(step)]['all32']+=all(gates.values())
