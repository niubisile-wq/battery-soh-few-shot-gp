...
def main():
    screen=HERE.parent/'results/m4_innovation_screen_v1'
    ...
    for s in frozen['selections']:
        root=Path(s['validation_root']);assert sa.digest(root/'result.json')==s['validation_result_sha256']
        result=json.loads((root/'result.json').read_text());ds,domain=s['fold'].split(':')
        for family in ('cv_innovation','uniform_innovation','cv_blend'):
            ...
            for parent in ('B','B1','B2','B3','B12','B13','B23','B123'):
                ...
                child=parent+'4'+('_'+family if family!='cv_innovation' else '')
    ...
    assert len(rows)==504
    focus=[r for r in rows if r['family']=='cv_innovation' and r['parent']=='B123']
    reversals=[r for r in focus if r['validation_delta_pp']< -1e-10 and r['query_delta_pp']>1e-10]
    out=HERE.parent/'results/m4_innovation_transfer_diagnosis_v1';out.mkdir(exist_ok=False)
    dump_csv(out/'folds.csv',rows)
    sa.write_json(out/'summary.json',dict(status='RETROSPECTIVE_DIAGNOSIS_ONLY',rows=504,
