"""Paired cached M2 source-risk draws, fixed M4 support-CV branch and all5 steps."""
import json
...
def main():
    results=HERE.parent/'results';source=results/'m4_support_cv_screen_v1';risk=results/'m3_waveform_stability_v2'
    ...
    out=results/'m4_support_cv_risk_sensitivity_v1';out.mkdir(exist_ok=False)
    sa.write_json(out/'protocol.json',dict(seeds=seeds,steps=steps,source_summary_sha256=sa.digest(source/'summary.json'),
        ...
        fixed='All GP fits,M4supportCV predictions and validation selections; global shrinkages same across every dataset/parent.',
    ...
    for s in frozen['selections']:
        ...
        gain=s['choices']['cv']['selected']['gain']
        for c in test:
            tail=Path('folds')/fold.replace(':','__')/'predictions'/(c.id+'.npz')
            with np.load(source/tail) as z:q=z['branch_cv'].copy();y=z['y'].copy()
