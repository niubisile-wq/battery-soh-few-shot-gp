...
def checked(root, fold, protocol):
    ...
    assert audit['code_sha256'] == sa.digest(HERE / 'audit_random_function.py')
    assert audit['helper_sha256'] == sa.digest(HERE / 'audit_conditional_mixed.py')
    assert audit['optimized_refits'] == 2 and max(audit['errors'].values()) < 1e-8
    assert all(sa.digest(HERE / n) == h for n, h in req['code_hashes'].items())
    assert result['prediction_sha256'] == sa.digest(root / 'predictions.joblib')
    assert len(result['models']) == 2 and {m['kernel'] for m in result['models']} == set(protocol['kernels'])
    warnings = []
    for model in result['models']:
        assert sa.digest(Path(model['path'])) == model['sha256']
        if not model['optimization']['success']:
            warnings.append(dict(kernel=model['kernel'], optimization=model['optimization']))
    assert warnings == audit['optimizer_failures']
    return dict(fold=fold, root=str(root), status='VALIDATION_REPLAY_AUDITED', optimizer_failures=warnings,
...
def run(fold, out, protocol):
    ...
    try:
        ...
        for script, args in [('random_function_pilot.py', ['--fold', fold, '--out', str(branch)]),
                             ('audit_random_function.py', ['--root', str(branch)])]:
...
def main():
    results = HERE.parent / 'results'; pilot = results / 'm4_random_function_pilot_v1'
    ...
    out = results / 'm4_random_function_batch_v1'; out.mkdir(exist_ok=False)
    ...
    sa.write_json(out / 'request.json', dict(folds=folds, workers=3, protocol=protocol, reused=done,
        source_batch_sha256=sa.digest(source), code_hashes={**req['code_hashes'], **{n:sa.digest(HERE / n) for n in
            ('batch_random_function.py', 'audit_random_function.py', 'audit_conditional_mixed.py')}},
    ...
    assert len(done) == 21
    sa.write_json(out / 'result.json', dict(status='ALL_VALIDATION_REPLAY_AUDITED' if all(r['status'] == 'VALIDATION_REPLAY_AUDITED' for r in done) else 'BATCH_HAS_TASK_FAILURES',
