...
def main():
    results=HERE.parent/'results';root=results/'m4_uniform_risk_sensitivity_v1'
    source=results/'m4_evidence_screen_v1';risk=results/'m3_waveform_stability_v2'
    ...
    assert sa.digest(HERE/'uniform_risk_sensitivity.py')==protocol['code_sha256']
    ...
    gains={s['fold']:s['choices']['uniform']['selected']['gain'] for s in choices}
    ...
    for ds in ('XJTU','MATR','Tongji'):
        for c in sa.load_cells(ds):
            ...
            with np.load(source/'folds'/folder/'predictions'/(c.id+'.npz')) as z:
                y=z['y'].copy();q=z['branch_uniform'].copy()
