"""Freeze evidence/uniform original-validation choices with cached branch controls."""
import json
...
def main():
    ...
    protocol=json.loads((HERE/'evidence_protocol.json').read_text())
    out=results/'m4_evidence_validation_v1';out.mkdir(exist_ok=False)
    sa.write_json(out/'request.json',dict(protocol=protocol,source_audit_sha256=sa.digest(source/'verification.json'),
        source_result_sha256=sa.digest(source/'result.json'),
        code_hashes={n:sa.digest(HERE/n) for n in ('validate_evidence.py','evidence_mixed.py','evidence_protocol.json','conditional_mixed.py')}))
    ...
    with threadpool_limits(limits=1):
        for entry in previous['selections']:
            ...
            for spec in old['models']:
                ...
                model=joblib.load(spec['path']);adapter=EvidenceMixed(model)
                for c in val:
                    ...
                    for family in protocol['families']:
                        key=family+'__'+kernel;a=w['private'] if family=='evidence' else .5
            ...
            sa.write_json(folder/'result.json',dict(fold=fold,selected=selected,trials=trials,rows=rows,models=old['models'],
                ...
                prediction_sha256=sa.digest(folder/'predictions.joblib'),weight_boundary=boundary))
            done.append(dict(fold=fold,root=str(folder),selected=selected,result_sha256=sa.digest(folder/'result.json')))
            sa.write_json(out/'progress.json',dict(completed=done,total=21));print(fold,'evidence/uniform selected',flush=True)
