"""Freeze all21 CV/uniform choices then score matched full16 ablations."""
import json
...
def group_name(g,family):
    return g+('_'+family if family!='cv' and '4' in g else '')
...
def main():
    ...
    with threadpool_limits(limits=1):
        for s in selections:
            ...
            for family,choice in s['choices'].items():
                ...
                adapters[family]=SupportCVMixed(model,family)
            ...
            for c in test:
                ...
                for family,adapter in adapters.items():
                    ...
                    pp['branch_'+family]=q
                with np.load(results/'m4_evidence_screen_v1'/tail) as oldpred:
                    for g in PARENTS:bias_error=max(bias_error,float(np.max(abs(pp[g+'4_uniform']-oldpred[g+'4_uniform']))))
    ...
    assert len(rows)==9490 and max(boundary,bias_error)<1e-8
    ...
    sa.write_json(out/'summary.json',dict(status='SCREEN_COMPLETE_NOT_ADOPTED',rows=len(rows),table=table,failures=failures,gates=gates,
        boundary=boundary,uniform_replay_error=bias_error,parent_metric_error=parent_error,
