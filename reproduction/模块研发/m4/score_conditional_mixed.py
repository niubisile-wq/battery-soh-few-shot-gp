"""Freeze63 validation choices, score all three full ablation families."""
...
def group_name(g,family):
    return g+('_'+family if family not in ('private','evidence') and '4' in g else '')


def main():
    ap=argparse.ArgumentParser();ap.add_argument('--evidence',action='store_true');args=ap.parse_args()
    variant='evidence' if args.evidence else 'conditional_mixed'
    ...
    if args.evidence:assert audit['helper_sha256']==sa.digest(HERE/'audit_conditional_mixed.py')
        ...
        for s in selections:
            ...
            for family,choice in s['choices'].items():
                ...
                if args.evidence:
                    ...
                    adapters[family]=EvidenceMixed(model,family)
                else:adapters[family]=model if family=='bias' else ConditionalMixed(model,family=='private')
            ...
            for c in test:
                ...
                for family,adapter in adapters.items():
                    ...
                    if args.evidence:
                        w=adapter.weights(sa.inference_view(c))
                        weight_rows.append(dict(fold=s['fold'],cell_id=c.id,family=family,gain=gain,**w))
                    for g in PARENTS:pp[group_name(g+'4',family)]=pp[g]+gain*(q-pp[g])
                    pp['branch_'+family]=q
                if not args.evidence:
                    with np.load(results/'m4_mixed_effects_screen_v1'/tail) as oldpred:
                        for g in PARENTS:bias_error=max(bias_error,float(np.max(abs(pp[g+'4_bias']-oldpred[g+'4']))))
    ...
    assert len(rows)==(9490 if args.evidence else 12775) and max(boundary,bias_error)<1e-8
    ...
    if args.evidence:dump_csv(out/'weights.csv',weight_rows)
    sa.write_json(out/'summary.json',dict(status='SCREEN_COMPLETE_NOT_ADOPTED',rows=len(rows),table=table,failures=failures,gates=gates,
        boundary=boundary,bias_replay_error=bias_error,parent_metric_error=parent_error,
        limits='Repeated development cohorts. Independent scoring audit,paired intervals and source-risk stability pending.'))
